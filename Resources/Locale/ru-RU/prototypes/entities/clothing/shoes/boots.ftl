ent-ClothingShoesBootsWork = рабочие ботинки
    .desc = Инженерные рабочие ботинки на шнуровке, для настоящих работяг.
    .suffix = { "" }
ent-ClothingShoesBootsJack = подкованные сапоги
    .desc = Обувь службы безопасности Nanotrasen, для боевых операций или боевых действий. Борьба - всё, борьба - всегда.
    .suffix = { "" }
ent-ClothingShoesBootsSalvage = ботинки исследователя
    .desc = Бтинки со стальными носками для дальних походов.
    .suffix = { "" }
ent-ClothingShoesBootsPerformer = ботинки артиста
    .desc = Эти ботинки обеспечивают отличное сцепление с поверхностью, когда вы выходите на сцену.
    .suffix = { "" }
ent-ClothingShoesBootsCombat = армейские ботинки
    .desc = Надёжные армейские ботинки, для боевых операций или боевых действий. Борьба - всё, борьба - всегда.
    .suffix = { "" }
ent-ClothingShoesBootsLaceup = шнурованные туфли
    .desc = Последний писк моды, и поставляются сразу отполированными!
    .suffix = { "" }
ent-ClothingShoesBootsMerc = ботинки наёмника
    .desc = Ботинки, прошедшие через множество конфликтов и доказавшие свою боевую надежность.
ent-ClothingShoesBootsWinter = зимние ботинки
    .desc = Пушистые сапожки помогут пережить даже самую холодную зиму.
ent-ClothingShoesBootsCowboyWhite = белые кожаные сапоги
    .desc = У них есть шпоры, которые звенят и/или раздражают.
ent-ClothingShoesBootsCowboyBlack = чёрные кожаные сапоги
    .desc = У них есть шпоры, которые звенят и/или раздражают.
ent-ClothingShoesBootsCowboyBrown = коричневые кожаные сапоги
    .desc = У них есть шпоры, которые звенят и/или раздражают.
ent-ClothingShoesBootsCowboyFancy = модные кожаные сапоги
    .desc = У них есть шпоры, которые звенят и/или раздражают.
ent-ClothingShoesBootsWinterWeb = зимние ботинки из паутины
    .desc = Сапоги из плотной паутины помогут пережить даже самую холодную зиму.
ent-ClothingShoesBootsWinterCargo = зимние ботинки грузчика
    .desc = Пушистые сапоги, которые помогут пережить даже самую холодную зиму.
ent-ClothingShoesBootsWinterEngi = зимние ботинки инженера
    .desc = Пушистые сапоги, которые помогут пережить даже самую холодную зиму.
ent-ClothingShoesClownBanana = банановые туфли клоуна
    .desc = Когда юмор и обувь поднимаются на новый уровень абсурда.
ent-ClothingShoesBootsWinterMed = зимние ботинки врача
    .desc = Пушистые сапоги, которые помогут пережить даже самую холодную зиму.
ent-ClothingShoesBootsWinterSec = зимние ботинки СБ
    .desc = Пушистые сапоги, которые помогут пережить даже самую холодную зиму.
ent-ClothingShoesBootsWinterSci = зимние ботинки учёного
    .desc = Пушистые сапоги, которые помогут пережить даже самую холодную зиму.
ent-ClothingShoesBootsWinterSyndicate = зимние ботинки синдиката
    .desc = Пушистые сапоги, которые помогут пережить даже самую холодную зиму.
ent-ClothingShoesHighheelBoots = туфли с высоким каблуком
    .desc = самая неудобная обувь на свете. Тот кто это изобрёл должен погибнуть в муках
ent-ClothingShoesBootsMagSci = магнитные сапоги
    .desc = Магнитные сапоги, используемые во время работы вне корабля, чтобы оставаться надёжно прикреплённым к поверхности.
ent-ClothingShoesBootsSpeed = скороходы
    .desc = Высокотехнологичные ботинки, сотканные из квантовых волокон, способные преобразовывать электричество в чистую скорость!
ent-ClothingShoesSnakeskinBoots = ботинки из кожи карпа
    .desc = Сапоги из высококлассной кожи карпа, вам будут завидовать все окружающие.
ent-ClothingShoesSkates = ролики
    .desc = Время покататься!
ent-ClothingShoesBootsMoon = лунные ботинки
    .desc = Специальные антигравитационные ботинки, разработанные с использованием специальной смеси лунного геля. Поставляется из Нидерландов.
